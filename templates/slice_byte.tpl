	...
	{{- if .DeepChecks.IsRef -}}
		...
		if *v < {{.DeepChecks.Min}} {
			errs = append(errs, fmt.Sprintf("{{.ErrorFieldName}}_%d::min_length_is::{{.DeepChecks.Min}}", i))
		}
		{{- end -}}
		{{- if .DeepChecks.CheckMax }}
		if *v > {{.DeepChecks.Max}} {
			errs = append(errs, fmt.Sprintf("{{.ErrorFieldName}}_%d::max_length_is::{{.DeepChecks.Max}}", i))
		}
		{{- end -}}
		{{- if .DeepChecks.CheckLen }}
		if *v != {{.DeepChecks.Len}} {
			errs = append(errs, fmt.Sprintf("{{.ErrorFieldName}}_%d::length_is_not::{{.DeepChecks.Len}}", i))
		}
		{{- end -}}
	{{- else -}}
		{{- if .DeepChecks.CheckMin }}
